...
void main() {
  ...
  final List<Article> changedArticlesList = [
    const Article(
      id: 1,
      title: 'Title 1',
      content: 'Contetn 1',
  ...
  group('ArticleBloc', () {
    ...
    blocTest(
      'emits Loaded when toggleing existing article',
      build: () {
        when(() => mockArticleRepository.getArticles()).thenAnswer(
          (_) async => articlesList,
        );
    ...
    blocTest(
      'emits LoadedWithError when toggleing not existing article',
      build: () {
        when(() => mockArticleRepository.getArticles()).thenAnswer(
          (_) async => articlesList,
        );
